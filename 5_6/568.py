...
with webdriver.Chrome(options=options) as webdriver:
    ...
    for cookie in cookies:
        ...
        webdriver.refresh()
        time.sleep(1)
        age = int(webdriver.find_element(By.ID, 'age').text.split()[-1])
        skills = len(webdriver.find_elements(By.CSS_SELECTOR, '#skillsList li'))
        data.append((age, skills, cookie['value']))
        webdriver.delete_all_cookies()
print(sorted(data, key=lambda x: (x[0], -x[1]))[0])
